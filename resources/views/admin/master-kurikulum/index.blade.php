...
@section('title', 'Admin | Master Kurikulum')
@section('subheader', 'Master Kurikulum')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card-box table-responsive">
                ...
                <table id="master_kurikulum_table" class="table table-bordered table-bordered dt-responsive nowrap">
                    <thead>
                        <tr>
                            <th width="10%">No</th>
                            <th>Nama</th>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="master_kurikulum_form" method="POST" data-parsley-validate novalidate>
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="kode" id="kode" parsley-trigger="change" required
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group row">
                        <label for="detail_kode" class="control-label col-md-4">Kode </label>
                        <div class="col-md-8">
...
@section('js')
    ...
    <script>
        $(document).ready(function(){
            var dataTables = $('#master_kurikulum_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.master-kurikulum.index') }}",
                },
                columns:[
                    {
                        data: 'DT_RowIndex'
        ...
        $(document).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('admin.master-kurikulum.detail', ['id' => ":id"]) }}";
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    $('#detail_kode').text(data.result.kode);
                    $('#detail').modal('show');
                }
            });
        });
        $('#create').click(function(){
            $('#master_kurikulum_form')[0].reset();
        ...
        $('#master_kurikulum_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('admin.master-kurikulum.store') }}",
                    method: "POST",
                    data: $(this).serialize(),
                    dataType: "json",
                    beforeSend: function()
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.errors)
                        {
                            ...
                            $('#master_kurikulum_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#master_kurikulum_table').DataTable().ajax.reload();
                        }
                        if(data.success)
                        {
            ...
            {
                $.ajax({
                    url: "{{ route('admin.master-kurikulum.update') }}",
                    method: "POST",
                    data: $(this).serialize(),
                    dataType: "json",
                    beforeSend: function(){
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('admin.master-kurikulum.edit', ['id' => ":id"]) }}";
            url = url.replace(":id", id);
            $.ajax({
                url: url,
                dataType: "json",
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('admin.master-kurikulum.destroy', ['id' => ":id"]) }}";
